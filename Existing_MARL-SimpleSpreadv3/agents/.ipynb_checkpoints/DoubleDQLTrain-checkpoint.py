...
class MultiAgentDoubleDQL:
    ...
    def _initialize_networks(self):
        ...
        agents = {}
        for i in range(3):
            agents[f"agent_{i}"] = DoubleQNetwork(obs_dim, action_dim)
        return agents
    ...
    def _save_model(self, agent_id, name_prefix, directory="./double_dqn_models/"):
    ...
    def _save_rewards(self, rewards, filename="rewards", directory="./double_dqn_models/"):
        os.makedirs(directory, exist_ok=True)
    ...
    def train_agents(self, total_episodes):
        ...
        np.save(os.path.join("./double_dqn_models/", "epsilon_history.npy"), self.epsilon_history)
        self.env.close()
        return all_rewards
