...
class MultiAgentDQL:
    ...
    def _initialize_networks(self):
        ...
        agents = {}
        for i in range(3):
            agents[f"agent_{i}"] = QNetwork(obs_dim, action_dim)
        return agents
    ...
    def _save_model(self, agent_id, name_prefix, directory="./dqn_models/"):
    ...
    def _save_rewards(self, rewards, filename="rewards", directory="./dqn_models/"):
        os.makedirs(directory, exist_ok=True)
        agent_ids = sorted(rewards[0].keys())
        reward_array = np.array([[ep[aid] for aid in agent_ids] for ep in rewards])
        npy_path = os.path.join(directory, f"{filename}")
        np.save(npy_path, reward_array)
    def train_agents(self, total_episodes):
        ...
        self._save_rewards(all_rewards, filename="final_rewards.npy")
        np.save(os.path.join("./dqn_models/", "epsilon_history.npy"), self.epsilon_history)
        self.env.close()
        return all_rewards
    def _learn_from_batch(self, batch, agent_id):
        ...
        for s, a, s_next, r, _ in batch:
            ...
            with torch.no_grad():
                max_q_next = target_net(next_state_tensor).max()
